...
export const VI_PLAIN: Record<string, string> = {
  ...
  "供应商配置总开关已关闭；当前只保存配置，不写入 Codex live 文件": "The provider configuration master switch is off; changes are only saved, not written to Codex live files",
  ...
  "健康检查": "Health check",
  "先添加至少 1 个已填写 Base URL / Key 的 API 供应商，再创建聚合供应商。": "Add at least 1 API provider with a Base URL / Key before creating an aggregate provider.",
  ...
  "关闭后本工具不会在手动切换时写入 Codex 的 config.toml / auth.json；启动 Codex 时始终不会自动改这些文件。": "When off, this tool won't write Codex's config.toml / auth.json on manual switches; it never auto-modifies these files when launching Codex.",
  "兼容增强": "Compatible enhancement",
  "内容来自 BigPizzaV3/Ad-List，分为赞助商推荐和普通推荐。": "Content comes from BigPizzaV3/Ad-List, split into sponsor recommendations and regular recommendations.",
  "内置": "Built-in",
  ...
  "取消": "Cancel",
  "只保留非 MCP、Skills、Plugins 的跨供应商配置；工具与插件在独立页面管理。": "Keeps only cross-provider config that isn't MCP/Skills/Plugins; tools and plugins are managed on a separate page.",
  "只填写表头下面的内容，例如：\ncommand = \"npx\"\nargs = [\"-y\", \"@upstash/context7-mcp\"]": "Fill in only the content under the header, for example:\ncommand = \"npx\"\nargs = [\"-y\", \"@upstash/context7-mcp\"]",
  "只能勾选已填写 Base URL / Key 的 API 供应商，聚合供应商不会作为成员。": "Only API providers with a Base URL / Key can be selected; aggregate providers can't be members.",
  ...
  "官方混合 API 不应在 auth.json 中保存 OPENAI_API_KEY。请清理此供应商的 auth.json 后再切换。": "Official mixed API should not store OPENAI_API_KEY in auth.json. Clear this provider's auth.json before switching.",
  "官方登录": "Official login",
  ...
  "已安装": "Installed",
  "已归档": "Archived",
  "已打开聚合供应商详情；请先添加或完善至少 1 个普通 API 供应商的 Base URL / Key，再勾选为成员。": "Opened aggregate provider details; first add or complete the Base URL / Key of at least 1 regular API provider, then select it as a member.",
  "已找到": "Found",
  "已按此供应商使用官方登录，并混入 API Key；Codex增强已设为兼容增强。": "Using official login for this provider with the API Key mixed in; Codex enhancements set to compatible.",
  ...
  "开启": "On",
  "开启后，通过 Codex++ 启动 Codex 前自动整理一次旧对话的归属标记。": "When on, tidies up ownership tags of old conversations once before launching Codex through Codex++.",
  ...
  "当前不会写入 Codex config.toml / auth.json。打开供应商配置总开关后再切换。": "Codex config.toml / auth.json won't be written right now. Turn on the provider configuration master switch before switching.",
  "当前为": "Currently ",
  "当前为兼容增强模式，插件市场解锁不会启用；其他页面功能仍可用。": "Currently in compatible enhancement mode; plugin marketplace unlock is not enabled, but other page features still work.",
  "当前会话": "Current session",
  "当前使用中：打开时从 ~/.codex/auth.json 回填，保存后会作为此供应商 auth 存档": "Currently in use: backfilled from ~/.codex/auth.json when opened; saving stores it as this provider's auth archive",
  ...
  "当前未登录官方账号；切到官方登录模式后仍需要先在 Codex/ChatGPT 登录。": "Not logged into an official account; after switching to official login mode you still need to log in via Codex/ChatGPT first.",
  "当前未登录官方账号；官方登录混入 API Key 需要先登录官方账号。": "Not logged into an official account; official login with a mixed-in API Key requires logging into an official account first.",
  ...
  "按对话轮转会让同一对话尽量保持固定成员，降低上下文漂移。": "Per-conversation round-robin keeps the same conversation on a fixed member where possible, reducing context drift.",
  ...
  "按请求轮转会逐请求切换成员，适合供应商能力接近的场景。": "Per-request round-robin switches members request by request, suited to providers with similar capabilities.",
  "接入模式": "Access mode",
  ...
  "权重轮转会读取每个成员的权重值，权重越高的成员获得更多请求。": "Weighted round-robin reads each member's weight; members with higher weight receive more requests.",
  ...
  "检测到来自网页的供应商配置导入请求，确认后会写入本机 Codex++ 管理工具。": "A provider config import request from the web was detected; confirming writes it to this machine's Codex++ manager.",
  "概览": "Overview",
  ...
  "正常": "OK",
  "此上游会通过本地 127.0.0.1:57321 转成 Responses API，需要从 Codex++ 启动 Codex。": "This upstream is converted to the Responses API via local 127.0.0.1:57321, which requires launching Codex from Codex++.",
  "此供应商会保留官方登录模式，并把请求混入当前 API Key；Codex增强仍使用兼容模式。": "This provider keeps official login mode and mixes the current API Key into requests; Codex enhancements stay in compatible mode.",
  "此供应商会切回官方登录模式，使用 ChatGPT 官方账号，不写入 API Key。": "This provider switches back to official login mode using the official ChatGPT account, without writing an API Key.",
  "此供应商会同时写入 config.toml 和 auth.json；API Key 也会注入到 provider bearer token。": "This provider writes both config.toml and auth.json; the API Key is also injected into the provider bearer token.",
  ...
  "状态": "Status",
  "独立管理 Codex 的 MCP、Skills、Plugins；切换任意供应商都会带上。": "Manage Codex's MCP, Skills and Plugins independently; they carry over whenever you switch providers.",
  ...
  "纯 API 配置未完整写入：请检查此供应商是否有 OPENAI_API_KEY，且 config.toml 是否包含 model_provider / provider / base_url。": "Pure API config is incomplete: check whether this provider has OPENAI_API_KEY and whether config.toml includes model_provider / provider / base_url.",
  ...
  "聚合供应商只保存成员和策略配置，成员来自已有 API 供应商；切为当前后会通过本地协议代理轮转请求。": "Aggregate providers only store member and strategy config drawn from existing API providers; once active, requests rotate through the local protocol proxy.",
  "聚合供应商至少需要勾选 1 个已填写 Base URL / Key 的 API 供应商。": "An aggregate provider needs at least 1 selected API provider with a Base URL / Key filled in.",
  ...
  "运行中": "Running",
  "返回列表": "Back to list",
  "这些变量可能覆盖当前供应商写入的 config.toml / auth.json；CODEX_HOME 不会被清理。": "These variables may override the config.toml / auth.json written by the current provider; CODEX_HOME won't be cleaned up.",
  ...
  "远程脚本": "Remote scripts",
  "适合官方登录或官方混入 API Key；保留会话删除、导出、项目移动和用户脚本，关闭插件市场相关增强。": "Suited to official login or official mixed-in API Key; keeps session deletion, export, project move and user scripts, while disabling plugin-marketplace-related enhancements.",
  "适合纯 API；启用插件市场、会话删除导出、项目移动等全部页面能力。": "Suited to pure API; enables all page capabilities including plugin marketplace, session deletion/export and project move.",
  ...
  "默认启动 Codex 时使用的模型名，请勿带后缀；上下文窗口请在下方「模型列表」中按模型单独配置。": "The model name used by default when launching Codex; don't include a suffix. Configure context windows per model in the “Model list” below.",
  ...
  "；这些条目独立于供应商保存，会写入所有供应商切换后的 config.toml。": "; these entries are saved independently of providers and written to config.toml after switching to any provider.",
};
...
export const VI_TEMPLATE: Record<string, string> = {
  ...
  "{0} 条": "{0} item(s)",
  "供应商 {0}": "Provider {0}",
  "供应商「{0}」缺少独立 config.toml，已停止切换，避免继续显示上一套配置文件。请先在该供应商详情里保存 config.toml。": "Provider “{0}” is missing its own config.toml, so the switch was stopped to avoid showing the previous config files. Save config.toml in this provider's details first.",
  ...
  "已同步到 {0}：修复 {1} 个会话文件，更新 {2} 行索引{3}。": "Synced to {0}: repaired {1} session file(s), updated {2} index row(s){3}.",
  "已安装 {0}": "Installed {0}",
  ...
  "聚合供应商已配置为{0}，包含 {1} 个成员；真实对话会走本地代理轮转。": "The aggregate provider is configured as {0} with {1} member(s); real conversations rotate through the local proxy.",
...
export const VI_BACKEND_PATTERNS: Array<[RegExp, string]> = [
  [new RegExp("^启动静默入口失败：(.+)$"), "Failed to launch silent entrypoint: $1"],
  [new RegExp("^无法启动 (.+?)：(.+)$"), "Cannot launch $1: $2"],
  [new RegExp("^设置已保存。(.*)$"), "Settings saved.$1"],
  [new RegExp("^保存设置失败：(.+)$"), "Failed to save settings: $1"],
  [new RegExp("^已读取 cc-switch Codex 供应商配置：(\\d+) 个。$"), "Read $1 cc-switch Codex provider config(s)."],
  [new RegExp("^读取 cc-switch 供应商配置失败：(.+)$"), "Failed to read cc-switch provider configs: $1"],
  [new RegExp("^已从 cc-switch 导入供应商配置：(\\d+) 个。$"), "Imported $1 cc-switch provider config(s)."],
  [new RegExp("^保存 cc-switch 供应商配置失败：(.+)$"), "Failed to save cc-switch provider configs: $1"],
  [new RegExp("^已导入供应商配置：(.+)。$"), "Imported provider config: $1."],
  [new RegExp("^供应商配置已存在：(.+)。$"), "Provider config already exists: $1."],
  [new RegExp("^导入供应商配置失败：(.+)$"), "Failed to import provider config: $1"],
  [new RegExp("^取消供应商导入失败：(.+)$"), "Failed to cancel provider import: $1"],
  [new RegExp("^已读取 (\\d+) 个本地会话。$"), "Read $1 local session(s)."],
  [new RegExp("^读取部分本地会话失败：(.+)$"), "Failed to read some local sessions: $1"],
  [new RegExp("^已读取 (\\d+) 个 Zed 远程项目。$"), "Read $1 Zed remote project(s)."],
  [new RegExp("^读取 Zed 远程项目失败。$"), "Failed to read Zed remote projects."],
  [new RegExp("^已在 Zed Remote 打开项目。$"), "Project opened in Zed Remote."],
  [new RegExp("^无法在 Zed Remote 打开项目。$"), "Cannot open project in Zed Remote."],
  [new RegExp("^移除 Zed 远程项目失败。$"), "Failed to remove Zed remote project."],
  [new RegExp("^供应商已同步一次：(\\d+) 个会话文件，(\\d+) 行索引，跳过 (\\d+) 个占用文件。$"), "Provider synced: $1 session file(s), $2 index row(s), skipped $3 locked file(s)."],
  [new RegExp("^供应商同步失败：(.+)$"), "Provider sync failed: $1"],
  [new RegExp("^推荐内容加载失败：(.+)$"), "Failed to load recommendations: $1"],
  [new RegExp("^脚本市场加载失败：(.+)$"), "Failed to load script marketplace: $1"],
  [new RegExp("^安装脚本失败：(.+)$"), "Failed to install script: $1"],
  [new RegExp("^脚本启停失败：(.+)$"), "Failed to toggle script: $1"],
  [new RegExp("^脚本删除失败：(.+)$"), "Failed to delete script: $1"],
  [new RegExp("^打开链接失败：(.+)$"), "Failed to open link: $1"],
  [new RegExp("^插件市场修复失败：(.+)$"), "Plugin marketplace repair failed: $1"],
  [new RegExp("^官方远端插件缓存修复失败：(.+)$"), "Official remote plugin cache repair failed: $1"],
  [new RegExp("^检查更新失败：(.+)$"), "Failed to check for updates: $1"],
  [new RegExp("^安装更新失败：(.+)$"), "Failed to install update: $1"],
  [new RegExp("^安装 watcher 失败：(.+)$"), "Failed to install watcher: $1"],
  [new RegExp("^移除 watcher 失败：(.+)$"), "Failed to remove watcher: $1"],
  [new RegExp("^启用 watcher 失败：(.+)$"), "Failed to enable watcher: $1"],
  [new RegExp("^禁用 watcher 失败：(.+)$"), "Failed to disable watcher: $1"],
  [new RegExp("^读取日志失败：(.+)$"), "Failed to read logs: $1"],
  [new RegExp("^重置设置失败：(.+)$"), "Failed to reset settings: $1"],
  [new RegExp("^重置图片覆盖层失败：(.+)$"), "Failed to reset image overlay: $1"],
  [new RegExp("^读取配置文件失败：(.+)$"), "Failed to read config file: $1"],
  [new RegExp("^删除环境变量失败：(.+)$"), "Failed to delete environment variables: $1"],
  [new RegExp("^保存配置文件失败：(.+)$"), "Failed to save config file: $1"],
  [new RegExp("^供应商切换失败：(.+)$"), "Provider switch failed: $1"],
  [new RegExp("^写入诊断日志失败：(.+)$"), "Failed to write diagnostics log: $1"],
  [new RegExp("^回填当前供应商配置失败：(.+)$"), "Failed to backfill current provider config: $1"],
  [new RegExp("^读取工具与插件列表失败：(.+)$"), "Failed to read tools & plugins list: $1"],
  [new RegExp("^读取 live 工具与插件失败：(.+)$"), "Failed to read live tools & plugins: $1"],
  [new RegExp("^保存工具与插件失败：(.+)$"), "Failed to save tools & plugins: $1"],
  [new RegExp("^读取 live config.toml 失败：(.+)$"), "Failed to read live config.toml: $1"],
  [new RegExp("^同步 live 工具与插件失败：(.+)$"), "Failed to sync live tools & plugins: $1"],
  [new RegExp("^创建 Codex 配置目录失败：(.+)$"), "Failed to create Codex config directory: $1"],
  [new RegExp("^写入 live config.toml 失败：(.+)$"), "Failed to write live config.toml: $1"],
  [new RegExp("^读取同步后的 live 工具与插件失败：(.+)$"), "Failed to read live tools & plugins after sync: $1"],
  [new RegExp("^删除工具与插件失败：(.+)$"), "Failed to delete tools & plugins: $1"],
  [new RegExp("^提取通用配置失败：(.+)$"), "Failed to extract common config: $1"],
  [new RegExp("^已向「(.+?)」用模型「(.+?)」发送 hi，HTTP (\\d+)。(.*)$"), "Sent hi to \"$1\" using model \"$2\", HTTP $3.$4"],
  [new RegExp("^测试「(.+?)」失败：(.+)$"), "Test \"$1\" failed: $2"],
  [new RegExp("^已从「(.+?)」获取 (\\d+) 个模型。$"), "Fetched $2 model(s) from \"$1\"."],
  [new RegExp("^从「(.+?)」获取模型失败：(.+)$"), "Failed to fetch models from \"$1\": $2"],
  [new RegExp("^切换完整中转配置失败：(.+)$"), "Failed to switch full relay config: $1"],
  [new RegExp("^写入中转配置失败：(.+)$"), "Failed to write relay config: $1"],
  [new RegExp("^写入聚合供应商配置失败：(.+)$"), "Failed to write aggregate provider config: $1"],
  [new RegExp("^切换纯 API 配置失败：(.+)$"), "Failed to switch to pure API config: $1"],
  [new RegExp("^写入纯 API 模式失败：(.+)$"), "Failed to write pure API mode: $1"],
  [new RegExp("^清除中转配置失败：(.+)$"), "Failed to clear relay config: $1"],
  [new RegExp("^未知配置文件类型：(.+)$"), "Unknown config file type: $1"],
  [new RegExp("^启动系统浏览器失败：(.+)$"), "Failed to launch system browser: $1"],
  [new RegExp("^诊断报告序列化失败：(.+)$"), "Failed to serialize diagnostics report: $1"],
  [new RegExp("^响应：(.+)$"), "Response: $1"],
];
